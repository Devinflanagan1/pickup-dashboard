...
# Reset the as-of widget's value whenever a new set of files is uploaded (detected
# via the uploaded file names+sizes). Without this, Streamlit's date_input widget
# "sticks" to whatever value it last showed in this browser session and ignores
# any new default — which is why picking a different file (or a code fix like
# this one) can otherwise appear to have no effect until you manually retype the date.
...
with st.sidebar:
    ...
    as_of = st.date_input(
        ...
        key="as_of_input",
        help="Defaults to today's real date every time you upload files, so "
             "'Days Left' always counts from now — not from a date inside the "
             "report. Change it here only if you want to look at pace as of a "
...
window = fa.selectbox("Booking window",
                      ...
                      index=6,
                      help="'All future dates' hides any date before your as-of date "
                           "(negative Days Left). Choose 'All dates (incl. past)' or use "
                           "the look-back option below to see past dates too.")

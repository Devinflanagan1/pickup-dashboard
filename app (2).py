...
def _signed_integer(v):
    return "–" if pd.isna(v) else f"{v:+,.0f}"


def _signed_points(v):
    return "–" if pd.isna(v) else f"{v * 100:+.1f} pts"


def _month_keys(as_of):
    base = pd.Timestamp(as_of).replace(day=1)
    return [base + pd.DateOffset(months=i) for i in range(3)]


def _scenario(revenue, rooms, available):
    rev = float(revenue.sum(min_count=1)) if revenue.notna().any() else np.nan
    rms = float(rooms.sum(min_count=1)) if rooms.notna().any() else np.nan
    avail = float(available.sum(min_count=1)) if available.notna().any() else np.nan
    return {
        "Rooms": rms,
        "Occupancy": rms / avail if avail and pd.notna(rms) else np.nan,
        "ADR": rev / rms if rms and pd.notna(rev) else np.nan,
        "RevPAR": rev / avail if avail and pd.notna(rev) else np.nan,
        "Revenue": rev,
    }


def _monthly_owner_data(raw, as_of):
    d = raw.copy()
    d["Occupancy Date"] = pd.to_datetime(d["Occupancy Date"], errors="coerce")
    d = d.dropna(subset=["Occupancy Date"])

    my_fcst_rev = _numeric(d, "My Forecast Revenue This Year")
    if my_fcst_rev.fillna(0).abs().sum() > 0:
        fcst_rev_col, fcst_source = "My Forecast Revenue This Year", "Your forecast"
    else:
        fcst_rev_col, fcst_source = "Forecasted Room Revenue This Year", "System forecast"

    my_fcst_rms = _numeric(d, "My Forecast Occupancy - Total This Year")
    fcst_rms_col = (
        "My Forecast Occupancy - Total This Year"
        if my_fcst_rms.fillna(0).abs().sum() > 0
        else "Occupancy Forecast - Total This Year"
    )

    output = []
    for month in _month_keys(as_of):
        sub = d[(d["Occupancy Date"].dt.year == month.year) &
                (d["Occupancy Date"].dt.month == month.month)]
        available = _numeric(sub, "Physical Capacity This Year")
        output.append({
            "label": month.strftime("%B %Y"),
            "otb": _scenario(_numeric(sub, "Booked Room Revenue This Year"),
                             _numeric(sub, "Occupancy On Books This Year"), available),
            "forecast": _scenario(_numeric(sub, fcst_rev_col),
                                  _numeric(sub, fcst_rms_col), available),
            "budget": _scenario(_numeric(sub, "Budget Room Revenue This Year"),
                                _numeric(sub, "Budget Occupancy - Total This Year"), available),
            "last_year": _scenario(_numeric(sub, "Booked Room Revenue STLY"),
                                   _numeric(sub, "Occupancy On Books STLY"), available),
        })
    return output, fcst_source


def _monthly_segment_data(raw, as_of):
    d = raw.copy()
    d["Occupancy Date"] = pd.to_datetime(d["Occupancy Date"], errors="coerce")
    d = d.dropna(subset=["Occupancy Date"])

    my_group = _numeric(d, "My Forecast Occupancy - Group This Year")
    group_fcst = ("My Forecast Occupancy - Group This Year"
                  if my_group.fillna(0).abs().sum() > 0
                  else "Occupancy Forecast - Group This Year")
    my_trans = _numeric(d, "My Forecast Occupancy - Transient This Year")
    trans_fcst = ("My Forecast Occupancy - Transient This Year"
                  if my_trans.fillna(0).abs().sum() > 0
                  else "Occupancy Forecast - Transient This Year")
    ...
    for month in _month_keys(as_of):
        ...
        rows.append({
            ...
            "Total OTB": _numeric(sub, "Occupancy On Books This Year").sum(),
            "Total Forecast": _numeric(sub, "Occupancy Forecast - Total This Year").sum(),
            "Total Budget": _numeric(sub, "Budget Occupancy - Total This Year").sum(),
            "Transient OTB": _numeric(sub, "Rooms Sold - Transient This Year").sum(),
            "Transient Forecast": _numeric(sub, trans_fcst).sum(),
            "Transient Budget": _numeric(sub, "Budget Occupancy - Transient This Year").sum(),
            "Group OTB": _numeric(sub, "Rooms Sold - Group This Year").sum(),
            "Group Forecast": _numeric(sub, group_fcst).sum(),
            "Group Budget": _numeric(sub, "Budget Occupancy - Group This Year").sum(),
        })
...
def _css():
    ...
    div[data-testid="stMetric"] {{background:white;border:1px solid #D9E2F3;
      border-radius:8px;padding:.65rem .8rem;box-shadow:0 1px 4px rgba(23,54,93,.08);}}
    div[data-testid="stMetric"] label {{color:{GRAY};font-weight:700;}}
    .dash-banner {{background:linear-gradient(90deg,{NAVY},{TEAL});color:white;
      border-radius:10px;padding:18px 24px;margin-bottom:12px;}}
    .dash-banner h1 {{margin:0;font-size:28px;}}
    .dash-banner p {{margin:4px 0 0;opacity:.9;}}
    .dash-section {{background:{NAVY};color:white;font-weight:800;text-transform:uppercase;
      letter-spacing:.04em;border-radius:6px;padding:8px 12px;margin:16px 0 8px;}}
    .status {{display:inline-block;padding:4px 9px;border-radius:999px;font-weight:700;margin-right:6px;}}
    .loaded {{background:{LIGHT_GREEN};color:#276221;}}
    .missing {{background:{LIGHT_RED};color:#842029;}}
    </style>
    """, unsafe_allow_html=True)


def _variance_style(v):
    if pd.isna(v):
        return ""
    return "color:#1A7F37;font-weight:700" if v >= 0 else "color:#C00000;font-weight:700"


def _render_month(month):
    metrics = ["Rooms", "Occupancy", "ADR", "RevPAR", "Revenue"]
    raw_rows, display_rows = [], []
    for metric in metrics:
        otb = month["otb"][metric]
        fcst = month["forecast"][metric]
        budget = month["budget"][metric]
        last_year = month["last_year"][metric]
        fvb = fcst - budget if pd.notna(fcst) and pd.notna(budget) else np.nan
        ovb = otb - budget if pd.notna(otb) and pd.notna(budget) else np.nan
        raw_rows.append({"Metric": metric, "Fcst vs Budget": fvb, "OTB vs Budget": ovb})
        if metric == "Occupancy":
            fmt, varfmt = _percent, _signed_points
        elif metric in ("ADR", "RevPAR", "Revenue"):
            fmt, varfmt = _money, _signed_money
        else:
            fmt, varfmt = _integer, _signed_integer
        display_rows.append({
            "Metric": metric,
            "On the Books": fmt(otb), "Forecast": fmt(fcst), "Budget": fmt(budget),
            "Last Year": fmt(last_year), "Fcst vs Budget": varfmt(fvb),
            "OTB vs Budget": varfmt(ovb),
        })

    raw_df = pd.DataFrame(raw_rows).set_index("Metric")
    display_df = pd.DataFrame(display_rows).set_index("Metric")
    sty = display_df.style
    sty = sty.apply(lambda _: [_variance_style(v) for v in raw_df["Fcst vs Budget"]],
                    subset=["Fcst vs Budget"])
    sty = sty.apply(lambda _: [_variance_style(v) for v in raw_df["OTB vs Budget"]],
                    subset=["OTB vs Budget"])
    st.dataframe(sty, use_container_width=True, height=225)


def render_dashboard(parts, report, as_of, pcdc_range=None):
    """Render the workbook-style Dashboard while leaving Pick-Up Detail unchanged."""
    _css()
    pcdc_text = ""
    if pcdc_range and all(pcdc_range):
        pcdc_text = f" | PCDC window: {pcdc_range[0]:%b %d} to {pcdc_range[1]:%b %d, %Y}"
    st.markdown(f"""
    <div class="dash-banner">
      <h1>The Compton | Revenue Dashboard</h1>
      <p>As of {pd.Timestamp(as_of):%A, %B %d, %Y}{pcdc_text}</p>
    </div>
    """, unsafe_allow_html=True)

    raw = parts.get("data")
    if raw is None:
        st.error("The Data Extract is required to render the Dashboard page.")
        return

    months, forecast_source = _monthly_owner_data(raw, as_of)
    current = months[0]

    st.markdown('<div class="dash-section">Current Month Performance</div>', unsafe_allow_html=True)
    a, b, c, d = st.columns(4)
    a.metric("OTB Revenue", _money(current["otb"]["Revenue"]),
             _signed_money(current["otb"]["Revenue"] - current["budget"]["Revenue"]))
    b.metric("Forecast Revenue", _money(current["forecast"]["Revenue"]),
             _signed_money(current["forecast"]["Revenue"] - current["budget"]["Revenue"]))
    c.metric("OTB Rooms", _integer(current["otb"]["Rooms"]),
             _signed_integer(current["otb"]["Rooms"] - current["budget"]["Rooms"]))
    d.metric("Forecast Rooms", _integer(current["forecast"]["Rooms"]),
             _signed_integer(current["forecast"]["Rooms"] - current["budget"]["Rooms"]))

    e, f, g, h = st.columns(4)
    e.metric("OTB Occupancy", _percent(current["otb"]["Occupancy"]),
             _signed_points(current["otb"]["Occupancy"] - current["budget"]["Occupancy"]))
    f.metric("Forecast Occupancy", _percent(current["forecast"]["Occupancy"]),
             _signed_points(current["forecast"]["Occupancy"] - current["budget"]["Occupancy"]))
    g.metric("OTB ADR", _money(current["otb"]["ADR"]),
             _signed_money(current["otb"]["ADR"] - current["budget"]["ADR"]))
    h.metric("Forecast RevPAR", _money(current["forecast"]["RevPAR"]),
             _signed_money(current["forecast"]["RevPAR"] - current["budget"]["RevPAR"]))

    st.markdown('<div class="dash-section">Revenue Action Queue</div>', unsafe_allow_html=True)
    action_col = report["⚡ Action"].astype(str) if "⚡ Action" in report else pd.Series("", index=report.index)
    spike_col = report["🔥 Spike"].astype(str) if "🔥 Spike" in report else pd.Series("", index=report.index)
    actions = report[action_col.str.contains("Not #1|Too close|At ceiling", regex=True, na=False)].copy()
    spikes = report[spike_col.str.strip().ne("")].copy()
    leading = report[action_col.str.contains("Leading", na=False)].copy()
    q1, q2, q3 = st.columns(3)
    q1.metric("Dates Requiring Pricing Review", f"{len(actions):,}")
    q2.metric("Spike Dates", f"{len(spikes):,}")
    q3.metric("Dates Safely Leading", f"{len(leading):,}")

    left, right = st.columns([1.35, 1])
    with left:
        st.markdown("**Pricing review dates**")
        cols = [x for x in ["Date", "DOW", "Events", "⚡ Action", "Lead Gap vs Comp",
                            "BAR", "Comp Set Avg", "Rooms Left to Sell"] if x in actions.columns]
        if len(actions):
            st.dataframe(actions[cols].sort_values("Date").head(20).style.format({
                "BAR": "${:,.0f}", "Comp Set Avg": "${:,.0f}",
                "Lead Gap vs Comp": "{:+,.0f}", "Rooms Left to Sell": "{:,.0f}",
            }, na_rep=""), use_container_width=True, hide_index=True, height=300)
        else:
            st.success("No rate-leadership exceptions in the current report range.")
    with right:
        st.markdown("**Top pickup spikes**")
        cols = [x for x in ["Date", "DOW", "Events", "🔥 Spike",
                            "Rooms Sold | Total Hotel | Change", "Days Left"] if x in spikes.columns]
        if len(spikes):
            spike_view = spikes[cols].copy()
            if "Rooms Sold | Total Hotel | Change" in spike_view:
                spike_view = spike_view.sort_values("Rooms Sold | Total Hotel | Change", ascending=False)
            st.dataframe(spike_view.head(20).style.format({
                "Rooms Sold | Total Hotel | Change": "+{:,.0f}", "Days Left": "{:,.0f}",
            }, na_rep=""), use_container_width=True, hide_index=True, height=300)
        else:
            st.info("No spike dates in the current report range.")

    st.markdown('<div class="dash-section">Monthly Owner Recap</div>', unsafe_allow_html=True)
    st.caption(f"Forecast source: {forecast_source}. Each panel uses the full calendar month from the Data Extract.")
    month_tabs = st.tabs([month["label"] for month in months])
    for tab, month in zip(month_tabs, months):
        with tab:
            _render_month(month)

    st.markdown('<div class="dash-section">Rooms by Business Mix</div>', unsafe_allow_html=True)
    segment_df = _monthly_segment_data(raw, as_of)
    tabs = st.tabs(["Total Hotel", "Transient", "Group"])
    for tab, prefix in zip(tabs, ["Total", "Transient", "Group"]):
        with tab:
            show = pd.DataFrame({
                "Month": segment_df["Month"],
                "OTB Rooms": segment_df[f"{prefix} OTB"],
                "Forecast Rooms": segment_df[f"{prefix} Forecast"],
                "Budget Rooms": segment_df[f"{prefix} Budget"],
            })
            show["Fcst vs Budget"] = show["Forecast Rooms"] - show["Budget Rooms"]
            show["OTB vs Budget"] = show["OTB Rooms"] - show["Budget Rooms"]
            sty = show.style.format({
                "OTB Rooms": "{:,.0f}", "Forecast Rooms": "{:,.0f}",
                "Budget Rooms": "{:,.0f}", "Fcst vs Budget": "{:+,.0f}",
                "OTB vs Budget": "{:+,.0f}",
            }, na_rep="")
            sty = sty.map(_variance_style, subset=["Fcst vs Budget", "OTB vs Budget"])
            st.dataframe(sty, use_container_width=True, hide_index=True)

    st.markdown('<div class="dash-section">Report Status</div>', unsafe_allow_html=True)
    statuses = [("PCDC", parts.get("pcdc") is not None), ("Data Extract", parts.get("data") is not None),
                ("Market Segment", parts.get("mseg") is not None), ("Rate Shop", parts.get("shop") is not None)]
    pills = []
    for label, ok in statuses:
        pills.append(f'<span class="status {"loaded" if ok else "missing"}">{label}: {"Loaded" if ok else "Missing"}</span>')
...
with st.sidebar:
    ...
    page = st.radio(
        "View",
        ["Dashboard", "Pick-Up Detail"],
        horizontal=True,
        key="main_view",
    )

# Build the existing report once. The dashboard reads the same calculations;
# the Pick-Up Detail page below remains unchanged.
...
if page == "Dashboard":
    render_dashboard(
        ...
        report=report,
        as_of=as_of,
        pcdc_range=(_pcdc_min, _pcdc_max),
    )
    st.stop()

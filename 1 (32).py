...
def build_report(parts, as_of, comp_method="Average", show_change="None", thr=None,
                 ...
                 restr_annotate=False, notes=None):
    ...
    # Free-text notes column (manual, carries forward via the prior export)
    _nds = df["Occupancy Date"].dt.strftime("%Y-%m-%d")
    r["📝 Notes"] = [str(notes.get(ds, "") or "") for ds in _nds]

    # ---- Enforce the exact original report column order ---- #
    lead = ["⚡ Action", "🔥 Spike", "📝 Notes", "DOW", "Date", "Days Left", "Events",
...
def to_excel(r, wb=None, grc_notes=None):
    ...
    `grc_notes` (optional) is a { datetime.date : note_text } dict of tentative
    group blocks from the Delphi GRC report; matching Date cells get a pop-out
    comment (red-triangle note) plus a soft flag fill.
    """
    ...
    for ri, (_, row) in enumerate(r.iterrows(), start=DATA0):
        ...
        for ci2, col in enumerate(r.columns, start=1):
            ...
            # center everything; left-align the long-text columns
            halign = "left" if col in ("Events", "Restrictions", "📝 Notes") else "center"
            cell.alignment = Alignment(horizontal=halign, vertical="center")

            # ---- tentative-group pop-out note on the Date cell (from Delphi GRC) ----
            if col == "Date" and grc_notes:
                _dk = v.date() if hasattr(v, "date") else v
                _txt = grc_notes.get(_dk)
                if _txt:
                    cm = Comment(_txt, "Delphi GRC — Tentative")
                    cm.width = 280
                    cm.height = 16 + 13 * (_txt.count("\n") + 1)
                    cell.comment = cm

            # ---- fill precedence: special > restriction > segment tint > weekend ----
            tint = seg_tint(col)
            if col == "Date" and grc_notes and grc_notes.get(
                    v.date() if hasattr(v, "date") else v):
                cell.fill = PF("FCE4D6")          # soft-orange flag for tentative dates
            elif col == "⚡ Action":
                af = _action_fill(v)
                if af is not None:
                    cell.fill = af
...
def compute_dashboard(cur_agg, last_agg, capacity, year,
                      ...
                      week_agg=None, lr_date=None, wk_date=None):
    """
...
def _dashboard_layout(title, freeze_label="Freeze Forecast",
                      ...
                      show_lr=True, show_week=False, lr_date=None, wk_date=None):
    ...
    same renderer drives every pickup view. When `lr_date`/`wk_date` are given,
    the comparison date is stamped into the pickup group headers (vs MM/DD)."""
    cols = []

    def _vs(base, d):
        return f"{base} (vs {pd.Timestamp(d):%m/%d})" if d is not None else base

    lr_last = _vs("Last Report OTB", lr_date)
    lr_pu = _vs("Pickup Since Last Report", lr_date)
    wk_base = _vs("7 Days Ago OTB", wk_date)
    wk_pu = _vs("7-Day Pickup", wk_date)
    ...
    if show_lr:
        add("lr_rn", _f_int, False, lr_last, "RN", _NF_INT)
        add("lr_adr", _f_money2, False, lr_last, "ADR", _NF_M2)
        add("lr_rev", _f_money0, False, lr_last, "Revenue", _NF_M0)
        add("pu_rn", _f_delta_int, True, lr_pu, "RN", _NF_DINT)
        add("pu_adr", _f_delta_money2, True, lr_pu, "ADR Chg", _NF_D2)
        add("pu_rev", _f_delta_money0, True, lr_pu, "Revenue", _NF_D0)
    if show_week:
        add("wk_rn", _f_int, False, wk_base, "RN", _NF_INT)
        add("wk_adr", _f_money2, False, wk_base, "ADR", _NF_M2)
        add("wk_rev", _f_money0, False, wk_base, "Revenue", _NF_M0)
        add("wpu_rn", _f_delta_int, True, wk_pu, "RN", _NF_DINT)
        add("wpu_adr", _f_delta_money2, True, wk_pu, "ADR Chg", _NF_D2)
        add("wpu_rev", _f_delta_money0, True, wk_pu, "Revenue", _NF_D0)
...
def render_dashboard_html(tables, header_line=None, colors=None,
    ...
    lr_date, wk_date = tables.get("_lr_date"), tables.get("_wk_date")
    for seg, title in [("Total", "Total OTB"),
        ...
        cols = _dashboard_layout(title, freeze_label, otbvar_label,
                                 ...
                                 lr_date=lr_date, wk_date=wk_date)
...
def build_combined_excel(tables, report, header_line=None,
                         show_lr=True, show_week=None,
                         rp_prod=None, rp_mapping=None, rp_header=None,
                         rp_cancellations=None, grc_notes=None):
    """Assemble ONE workbook containing the owner Dashboard, the DD Pick-Up
    Report, and (optionally) the Transient Rate Plan Production (+ Cancellations).
    Any section is optional — pass None to skip it.
    Sheet order: Dashboard, Pick-Up Report, Rate Plan Production, Cancellations,
    Rate Plan Map."""
    from openpyxl import Workbook
    wb = Workbook()
    ...
    if report is not None:
        to_excel(report, wb=wb, grc_notes=grc_notes)
    if rp_prod is not None and rp_mapping is not None:
        rateplan_to_excel(rp_prod, rp_mapping, header_line=rp_header, wb=wb,
                          cancellations=rp_cancellations)
...
def dashboard_tables_for_export(parts, as_of):
    ...
    # comparison dates (only known when a prior Compton export was uploaded)
    lr_date = (cmp_lr or {}).get("report_date") if (cmp_lr and cmp_lr.get("monthly")) else None
    wk_date = (cmp_wk or {}).get("report_date") if (cmp_wk and cmp_wk.get("monthly")) else None
    ...
    if lr_date is not None:
        gap_bits.append(f"vs last report {pd.Timestamp(lr_date):%m/%d/%Y}")
    if wk_date is not None and tables.get("_has_week"):
        gap_bits.append(f"vs 7-days-ago {pd.Timestamp(wk_date):%m/%d/%Y}")
...
def render_dashboard_page(parts, as_of):
    ...
    tables = compute_dashboard(cur_agg, last_agg, capacity, year,
                               data_extract=data, budget_df=budget_df,
                               freeze_df=freeze_df, week_agg=week_agg,
                               lr_date=(last_report_date if pickup_src == "uploaded file" else None),
                               wk_date=week_date)
    ...
    if dd_report is not None:
        # optionally fold in Rate Plan Production if a SynXis file was uploaded
        _rp_prod = _rp_map = _rp_hdr = _rp_canc = None
        _resv = parts.get("synxis_resv")
        if _resv is not None and len(_resv):
            _rp_map = st.session_state.get("rpp_map")
            if _rp_map is None:
                _rp_map = rate_plan_mapping_default()
            _rp_prod = compute_rate_plan_production(_resv, _rp_map, "All months")
            _rp_canc = cancellation_review_df(_resv, "All months")
            _rp_hdr = f"The Compton — Transient Rate Plan Production — All months"
        combined = build_combined_excel(
            tables, dd_report,
            header_line=hdr_plain, show_lr=show_lr, show_week=show_week,
            rp_prod=_rp_prod, rp_mapping=_rp_map, rp_header=_rp_hdr,
            rp_cancellations=_rp_canc, grc_notes=parts.get("grc_notes"))
        label = ("⬇️ Dashboard + DD + Rate Plan (one Excel)" if _rp_prod is not None
                 else "⬇️ Dashboard + DD (one Excel)")
        dl2.download_button(
            label, data=combined,
...
# =========================================================================== #
#  RATE PLAN PRODUCTION  — reproduces the workbook 'Transient Rate Plan
#  Production' tab from a SynXis HotelReservations export.
# =========================================================================== #
SEGMENT_ORDER = ["Retail", "Prepaid", "Package", "Promo", "Qualified Discount",
                 "TMC", "National", "Local", "Comp"]

RATE_PLAN_SEED = [
    ('FLEX999', 'Flexible Rate', 'Retail'),
    ('EXPECFLEX', 'Expedia Collect Flexible', 'Retail'),
    ('EXPHCFLEX', 'Expedia Hotel Collect Flexible', 'Retail'),
    ('BKFLEX999', 'Booking.com Flex', 'Retail'),
    ('', 'Hopper Flexible', 'Retail'),
    ('', 'Agoda Flexible', 'Retail'),
    ('PREPAY10', 'Advance Purchase', 'Prepaid'),
    ('', 'Agoda Prepay', 'Prepaid'),
    ('', 'Booking.com Pre-Paid', 'Prepaid'),
    ('EXPECPP10', 'Expedia Collect Pre-Pay', 'Prepaid'),
    ('', 'Expedia Hotel Collect Pre-Pay', 'Prepaid'),
    ('', 'Hopper Prepay', 'Prepaid'),
    ('', 'HotelTonight', 'Prepaid'),
    ('', 'A Taste of The Compton', 'Package'),
    ('EXPECPKG', 'Expedia Package Rate', 'Package'),
    ('', 'Grand Opening', 'Package'),
    ('', 'Inside Track Extras', 'Package'),
    ('', 'Ozark Valentines Getaway', 'Package'),
    ('PARK', 'Parking Pakage', 'Package'),
    ('CDINB', 'Cool Down In Bentonville', 'Package'),
    ('', 'Agoda LOS', 'Promo'),
    ('', 'Booking.com Promo', 'Promo'),
    ('', 'Booking.com Stay Longer and Save', 'Promo'),
    ('', 'GDS Promo', 'Promo'),
    ('', 'Hopper Member', 'Promo'),
    ('MOBILE', 'Mobile Rate', 'Promo'),
    ('AAA/CAA', 'AAA/CAA', 'Qualified Discount'),
    ('', 'Government Rate', 'Qualified Discount'),
    ('ORR', 'Ozark Resident Rate', 'Qualified Discount'),
    ('', 'Tablet Plus', 'Qualified Discount'),
    ('', 'Tablet Advance Purchase', 'Qualified Discount'),
    ('ABC', 'ABC PHP', 'TMC'),
    ('', 'ABOVE AND BEYOND', 'TMC'),
    ('ZGK', 'Amex GBT Rate', 'TMC'),
    ('TQV', 'Amex GBT Value Rate', 'TMC'),
    ('', 'BCD Travel Hotel Program', 'TMC'),
    ('', 'Breakfast Plus Program', 'TMC'),
    ('', 'CTM Global Hotel Program', 'TMC'),
    ('', 'CWT Business Rate', 'TMC'),
    ('', 'Direct Global', 'TMC'),
    ('', 'Direct Global Value', 'TMC'),
    ('', 'Global Program', 'TMC'),
    ('', 'Global Stays Advantage', 'TMC'),
    ('', 'Global Stays Hickory', 'TMC'),
    ('', 'GlobalStar Hotel Collection', 'TMC'),
    ('TL7', 'Internova Worldwide Program', 'TMC'),
    ('', 'OneSTAY', 'TMC'),
    ('', 'Radius Global Hotel Program', 'TMC'),
    ('', 'THOR Consortia Rate', 'TMC'),
    ('', 'THOR Promotional Rate', 'TMC'),
    ('', 'TRAVELSAVERS', 'TMC'),
    ('', 'World Travel Preferred', 'TMC'),
    ('', 'Flight Center Breakfast Plus', 'TMC'),
    ('', 'Global Star Hotel Collection Promo', 'TMC'),
    ('F6H', 'Flight Center Discount Rates', 'TMC'),
    ('', 'Hickory Promotional Rate', 'TMC'),
    ('AXTHCM', 'AMEX THC Expedia Collect', 'TMC'),
    ('', 'AMEX THC Hotel Collect', 'TMC'),
    ('AMEXTHC', 'Amex The Hotel Collection', 'TMC'),
    ('', 'BCD Entertainment Hotel Program', 'TMC'),
    ('MCKP', 'McKinsey Project Rate', 'TMC'),
    ('BCDMM', 'BCD Mid Market', 'TMC'),
    ('BAINC', 'Bain and Company', 'National'),
    ('WLM', 'Walmart', 'National'),
    ('', 'Bentonville Business Rate', 'Local'),
    ('', 'Ruz USA', 'Local'),
    ('', 'Mercy', 'Local'),
    ('', 'The Influencer Advantage', 'Local'),
    ('', 'Media Rate', 'Local'),
    ('', 'Friends of The Compton', 'Local'),
    ('', 'Arvest', 'Local'),
    ('', 'Rapha', 'Local'),
    ('', 'Indigo Road Hospitality Group', 'Local'),
    ('', 'Onepay', 'Local'),
    ('', 'Crenlo', 'Local'),
    ('', 'Heartland Forward', 'Local'),
    ('', 'Thaden School', 'Local'),
    ('UPLABS', 'UP.Labs', 'Local'),
    ('', 'Skylark Flex', 'Local'),
    ('', 'RCC Member Discount', 'Local'),
    ('', 'Tyson', 'Local'),
    ('', 'Nintendo', 'Local'),
    ('', 'Runway Group', 'Local'),
    ('', 'IMA', 'Local'),
    ('', 'Complimentary Rate', 'Comp'),
    ('', 'Wedding Celebration', 'Comp'),
    ('', 'Event Rate', 'Comp'),
    ('', 'Friends & Family', 'Comp'),
]


def rate_plan_mapping_default():
    """Editable master mapping: Rate Code -> Rate Plan Name -> Segment."""
    return pd.DataFrame(RATE_PLAN_SEED, columns=["Rate Code", "Rate Plan Name", "Segment"])


def parse_synxis_reservations(file):
    """Read a SynXis HotelReservations CSV/xlsx into tidy reservation rows.
    Returns ALL usable reservations (Confirmed AND Cancelled; the metadata /
    'Ignored' rows are dropped) with a `status` column plus review fields:
        status, confirm_no, guest, channel, sec_channel, code, category,
        arrival, depart, nights, rooms, rate, value, RN, Rev, ym, cancel_dt.
    Handles thousands separators in the rate/price fields."""
    ...
    c_status = col("Rez_Status_Desc")
    c_code = col("Rate_Type_Code")
    c_cat = col("Rate_Category_Name")
    c_nights = col("Nights_Qty")
    c_rooms = col("Room_Qty")
    c_rate = col("Rez_Avg_Rate_Amt")
    c_arr = col("Arrival_Dt")
    c_dep = col("Depart_Dt")
    c_conf = col("Confirm_No", "Confirmation_No", "Confirm_Number")
    c_guest = col("Guest_Nm", "Guest_Name")
    c_chan = col("Channel_Cd", "Channel")
    c_sec = col("Sec_Channel_Desc", "Secondary_Channel")
    c_stat_dt = col("Status_Dt", "Cancel_Dt", "Cancellation_Dt")
    c_val = col("Total_Price_For_Adult", "Total_Price")
    if c_code is None or c_nights is None or c_rate is None:
        return pd.DataFrame(columns=["status", "code", "category", "arrival",
                                     "nights", "rooms", "rate", "RN", "Rev", "ym"])

    def num(s, default=0.0):
        return pd.to_numeric(df[s].astype(str).str.replace(",", "", regex=False),
                             errors="coerce") if s else pd.Series(default, index=df.index)

    out = pd.DataFrame(index=df.index)
    out["status"] = (df[c_status].astype(str).str.strip().str.title()
                     if c_status else "Confirmed")
    out["confirm_no"] = df[c_conf].astype(str).str.strip() if c_conf else ""
    out["guest"] = df[c_guest].astype(str).str.strip() if c_guest else ""
    out["channel"] = df[c_chan].astype(str).str.strip() if c_chan else ""
    out["sec_channel"] = df[c_sec].astype(str).str.strip() if c_sec else ""
    out["code"] = df[c_code].astype(str).str.strip()
    out["category"] = df[c_cat].astype(str).str.strip() if c_cat else ""
    out["arrival"] = pd.to_datetime(df[c_arr], errors="coerce") if c_arr else pd.NaT
    out["depart"] = pd.to_datetime(df[c_dep], errors="coerce") if c_dep else pd.NaT
    out["cancel_dt"] = pd.to_datetime(df[c_stat_dt], errors="coerce") if c_stat_dt else pd.NaT
    out["nights"] = num(c_nights).fillna(0)
    out["rooms"] = num(c_rooms).fillna(1)
    out["rate"] = num(c_rate).fillna(0)
    out["value"] = num(c_val).fillna(0)
    out["RN"] = out["nights"] * out["rooms"]
    out["Rev"] = out["RN"] * out["rate"]
    out["ym"] = out["arrival"].dt.strftime("%Y-%m")

    # keep only real bookings we understand (Confirmed / Cancelled), drop the
    # metadata rows (Report_Criteria_*, NOT SPECIFIED) and 'Ignored'.
    keep = out["status"].isin(["Confirmed", "Cancelled", "Canceled"])
    out = out[keep & (out["code"] != "") & (out["code"].str.lower() != "nan")
              & (out["RN"] > 0)].copy()
    out["status"] = out["status"].replace({"Canceled": "Cancelled"})
    return out.reset_index(drop=True)


def confirmed_only(resv):
    """Confirmed reservations (what feeds the production report)."""
    if resv is None or len(resv) == 0:
        return resv
    return resv[resv["status"] == "Confirmed"].reset_index(drop=True)


def cancelled_only(resv):
    """Cancelled reservations, for PMS fee review (with confirmation numbers)."""
    if resv is None or len(resv) == 0:
        return resv.iloc[0:0] if resv is not None else resv
    return resv[resv["status"] == "Cancelled"].reset_index(drop=True)


CANCEL_COLUMNS = ["Confirm #", "Guest", "Channel", "Rate Code", "Category",
                  "Arrival", "Depart", "Room Nights", "ADR", "Value",
                  "Cancelled On", "Days Before Arrival"]


def cancellation_review_df(resv, month_label="All months"):
    """Tidy, sorted table of cancelled reservations for PMS review, including how
    far in advance of arrival each was cancelled (Days Before Arrival = arrival −
    cancelled date; negative = cancelled after arrival)."""
    canc = cancelled_only(resv)
    if canc is None or len(canc) == 0:
        return pd.DataFrame(columns=CANCEL_COLUMNS)
    if month_label and month_label != "All months":
        ym = _label_to_ym(month_label)
        canc = canc[canc["ym"] == ym]
    if len(canc) == 0:
        return pd.DataFrame(columns=CANCEL_COLUMNS)

    # lead time between the cancellation and the arrival date
    days_before = (canc["arrival"].dt.normalize()
                   - canc["cancel_dt"].dt.normalize()).dt.days

    canc = canc.assign(_dba=days_before).sort_values(["arrival", "confirm_no"])
    out = pd.DataFrame({
        "Confirm #": canc["confirm_no"].values,
        "Guest": canc["guest"].values,
        "Channel": canc["channel"].where(canc["sec_channel"].eq(""),
                                         canc["channel"] + " / " + canc["sec_channel"]).values,
        "Rate Code": canc["code"].values,
        "Category": canc["category"].values,
        "Arrival": canc["arrival"].dt.date.values,
        "Depart": canc["depart"].dt.date.values,
        "Room Nights": canc["RN"].values,
        "ADR": canc["rate"].round(2).values,
        "Value": canc["Rev"].round(2).values,
        "Cancelled On": canc["cancel_dt"].dt.date.values,
        "Days Before Arrival": canc["_dba"].astype("Int64").values,
    })
    return out.reset_index(drop=True)


def available_months(resv):
    """Sorted 'Mon YYYY' labels present in the reservations (by arrival)."""
    yms = sorted(m for m in resv["ym"].dropna().unique())
    labels = []
    for ym in yms:
        y, mo = ym.split("-")
        labels.append(f"{calendar.month_abbr[int(mo)]} {y}")
    return list(dict.fromkeys(labels))


def _label_to_ym(label):
    mon, yr = label.split()
    return f"{yr}-{list(calendar.month_abbr).index(mon):02d}"


def compute_rate_plan_production(resv, mapping_df, month_label="All months"):
    """Group CONFIRMED reservations into the rate-plan/segment layout.
    Returns dict: {'segments': {seg: [rows]}, 'seg_totals': {seg: row},
                   'grand': row, 'unmapped': [(code, category, RN, Rev)]}.
    Each `row` = {name, los, rn, adr, rev, bookings}."""
    resv = confirmed_only(resv)                 # production = confirmed only
    if month_label and month_label != "All months":
        ym = _label_to_ym(month_label)
        resv = resv[resv["ym"] == ym]

    code2name, code2seg = {}, {}
    roster = {seg: [] for seg in SEGMENT_ORDER}
    for _, m in mapping_df.iterrows():
        code = str(m["Rate Code"]).strip()
        name = str(m["Rate Plan Name"]).strip()
        seg = str(m["Segment"]).strip()
        if not name:
            continue
        if seg not in roster:
            roster[seg] = []
        if name not in roster[seg]:
            roster[seg].append(name)
        if code and code.lower() != "nan":
            code2name[code] = name
            code2seg[code] = seg

    # aggregate reservations by code
    agg = {}
    unmapped = {}
    for _, r in resv.iterrows():
        code = r["code"]
        rn, rev = float(r["RN"]), float(r["Rev"])
        if code in code2name:
            key = (code2seg[code], code2name[code])
            a = agg.setdefault(key, {"rn": 0.0, "rev": 0.0, "bk": 0})
            a["rn"] += rn; a["rev"] += rev; a["bk"] += 1
        else:
            u = unmapped.setdefault(code, {"category": r["category"], "rn": 0.0, "rev": 0.0})
            u["rn"] += rn; u["rev"] += rev

    def mkrow(name, a):
        rn = a["rn"]; rev = a["rev"]; bk = a["bk"]
        return {"name": name,
                "los": (rn / bk) if bk else np.nan,
                "rn": rn, "adr": (rev / rn) if rn else np.nan, "rev": rev,
                "bookings": bk}
    ...
    for seg in SEGMENT_ORDER:
        rows = []
        t_rn = t_rev = t_bk = 0.0
        for name in roster.get(seg, []):
            a = agg.get((seg, name))
            if a:
                rows.append(mkrow(name, a))
                t_rn += a["rn"]; t_rev += a["rev"]; t_bk += a["bk"]
            else:
                rows.append({"name": name, "los": np.nan, "rn": np.nan,
                             "adr": np.nan, "rev": np.nan, "bookings": 0})
        segments[seg] = rows
        seg_totals[seg] = {"name": "Total",
                           "los": (t_rn / t_bk) if t_bk else np.nan,
                           "rn": t_rn, "adr": (t_rev / t_rn) if t_rn else np.nan,
                           "rev": t_rev, "bookings": t_bk}
        g_rn += t_rn; g_rev += t_rev; g_bk += t_bk

    grand = {"los": (g_rn / g_bk) if g_bk else np.nan, "rn": g_rn,
             "adr": (g_rev / g_rn) if g_rn else np.nan, "rev": g_rev, "bookings": g_bk}
    unmapped_list = [(c, v["category"], v["rn"], v["rev"])
                     for c, v in sorted(unmapped.items())]
    return {"segments": segments, "seg_totals": seg_totals, "grand": grand,
            "unmapped": unmapped_list}


# --------------------------------------------------------------------------- #
#  Rendering (HTML for the app + Excel export)
# --------------------------------------------------------------------------- #
def _f_los(v):
...
def rateplan_css():
    return """
<style>
.rpp { font-family: Segoe UI, Arial, sans-serif; }
.rpp table { border-collapse: collapse; width: 100%; font-size: 12px; margin-bottom: 4px; }
.rpp th, .rpp td { padding: 2px 6px; white-space: nowrap; }
.rpp .seg { background:#4472C4; color:#fff; font-weight:700; text-align:left;
    border:1px solid #2F528F; }
.rpp .colh { background:#4472C4; color:#fff; font-weight:700; text-align:right;
    border:1px solid #2F528F; }
.rpp .tot td { background:#B4C7E7; font-weight:700; border:1px solid #9AB0DA; }
.rpp td.name { text-align:left; border:1px solid #E2E2E2; }
.rpp td.num  { text-align:right; border:1px solid #E2E2E2; }
.rpp tr.empty td { color:#B0B0B0; }
.rpp .hdrline { font-weight:700; font-size:13px; margin:6px 0; }
</style>
"""


def _block_html(seg, rows, total):
    h = ['<table>']
    h.append(f'<tr><th class="seg">{seg}</th>'
             '<th class="colh">LOS</th><th class="colh">Room Nights</th>'
             '<th class="colh">ADR</th><th class="colh">Revenue</th></tr>')
    h.append(f'<tr class="tot"><td>Total</td>'
             f'<td class="num">{_f_los(total["los"])}</td>'
             f'<td class="num">{_f_int(total["rn"])}</td>'
             f'<td class="num">{_f_money(total["adr"])}</td>'
             f'<td class="num">{_f_money(total["rev"])}</td></tr>')
    for r in rows:
        empty = r["rn"] is None or (isinstance(r["rn"], float) and pd.isna(r["rn"]))
        cls = ' class="empty"' if empty else ''
        h.append(f'<tr{cls}><td class="name">{r["name"]}</td>'
                 f'<td class="num">{_f_los(r["los"])}</td>'
                 f'<td class="num">{_f_int(r["rn"])}</td>'
                 f'<td class="num">{_f_money(r["adr"])}</td>'
                 f'<td class="num">{_f_money(r["rev"])}</td></tr>')
    h.append('</table>')
    return "".join(h)


def render_rateplan_html(prod, header_line=None):
    parts = [rateplan_css()]
    if header_line:
        parts.append(f'<div class="rpp"><div class="hdrline">{header_line}</div></div>')
    layout = [["Retail", "Prepaid", "Package"],
              ["Promo", "Qualified Discount", "TMC"],
              ["National", "Local", "Comp"]]
    parts.append('<div class="rpp">')
    for band in layout:
        parts.append('<table><tr>')
        for seg in band:
            parts.append('<td style="vertical-align:top;width:33%;border:none;padding:0 8px 0 0">')
            parts.append(_block_html(seg, prod["segments"].get(seg, []),
                                     prod["seg_totals"].get(seg, {})))
            parts.append('</td>')
...
def _write_cancellation_sheet(wb, canc_df, title="Cancellations",
                              header_line=None):
    """Write a cancellation-review table into `wb` (confirmation #, guest, dates,
    value, etc.) so cancellations can be checked in the PMS for fees owed."""
    from openpyxl.styles import Font, Alignment, PatternFill, Border, Side
    from openpyxl.utils import get_column_letter
    ...
    cols = list(canc_df.columns)
    for j, h in enumerate(cols, start=1):
        c = ws.cell(row=row, column=j, value=h)
        c.font = white; c.fill = PatternFill("solid", fgColor=RED)
        c.alignment = Alignment(horizontal="center", vertical="center")
    money = {"ADR", "Value"}
    for _, rr in canc_df.iterrows():
        row += 1
        for j, h in enumerate(cols, start=1):
            v = rr[h]
            cell = ws.cell(row=row, column=j,
                           value=(None if (isinstance(v, float) and pd.isna(v)) else v))
            cell.border = thin; cell.font = Font(size=9)
            if h in money:
                cell.number_format = '"$"#,##0.00'
            elif h in ("Room Nights", "Days Before Arrival"):
                cell.number_format = "#,##0"
            cell.alignment = Alignment(
                horizontal="left" if h in ("Guest", "Channel", "Category") else "center")
    # total row
    if len(canc_df):
        row += 1
        ws.cell(row=row, column=1, value="Total").font = Font(bold=True, size=9)
        for j, h in enumerate(cols, start=1):
            if h == "Room Nights":
                cell = ws.cell(row=row, column=j, value=float(canc_df[h].sum()))
                cell.number_format = "#,##0"; cell.font = Font(bold=True, size=9)
            elif h == "Value":
                cell = ws.cell(row=row, column=j, value=float(canc_df[h].sum()))
                cell.number_format = '"$"#,##0.00'; cell.font = Font(bold=True, size=9)
    widths = {"Confirm #": 16, "Guest": 26, "Channel": 20, "Rate Code": 12,
              "Category": 18, "Arrival": 11, "Depart": 11, "Room Nights": 12,
              "ADR": 11, "Value": 12, "Cancelled On": 13, "Days Before Arrival": 17}
    for j, h in enumerate(cols, start=1):
        ws.column_dimensions[get_column_letter(j)].width = widths.get(h, 12)
    ws.freeze_panes = f"A{(3 if header_line else 1) + 1}"
    return ws


def cancellations_to_excel(canc_df, header_line=None):
    """Standalone cancellation-review workbook."""
    from openpyxl import Workbook
    wb = Workbook()
    default_ws = wb.active
    _write_cancellation_sheet(wb, canc_df, title="Cancellations",
...
def rateplan_to_excel(prod, mapping_df, header_line=None, wb=None, cancellations=None):
    from openpyxl import Workbook
    from openpyxl.styles import Font, Alignment, PatternFill, Border, Side
    ...
    if standalone:
        wb = Workbook()
        ws = wb.active
        ws.title = "Rate Plan Production"
    else:
        ws = wb.create_sheet("Rate Plan Production")

    BLUE, LTBLUE = "4472C4", "B4C7E7"
    white = Font(color="FFFFFF", bold=True, size=10)
    boldf = Font(bold=True, size=10)
    reg = Font(size=10)
    ...
    layout = [["Retail", "Prepaid", "Package"],
              ["Promo", "Qualified Discount", "TMC"],
              ["National", "Local", "Comp"]]
    col_starts = [1, 7, 13]        # each block spans 5 cols + 1 gap

    for band in layout:
        band_top = row
        max_len = 0
        for bi, seg in enumerate(band):
            c0 = col_starts[bi]
            rows = prod["segments"].get(seg, [])
            total = prod["seg_totals"].get(seg, {})
            rr = band_top
            # header
            ws.cell(row=rr, column=c0, value=seg).font = white
            ws.cell(row=rr, column=c0).fill = PatternFill("solid", fgColor=BLUE)
            ws.cell(row=rr, column=c0).alignment = left
            for j, h in enumerate(["LOS", "Room Nights", "ADR", "Revenue"], start=1):
                cell = ws.cell(row=rr, column=c0 + j, value=h)
                cell.font = white; cell.fill = PatternFill("solid", fgColor=BLUE)
                cell.alignment = right
            rr += 1
            # total
            for j, key, fmt in [(0, "name", None), (1, "los", "0.0"),
                                (2, "rn", "#,##0"), (3, "adr", '"$"#,##0.00'),
                                (4, "rev", '"$"#,##0.00')]:
                v = "Total" if key == "name" else total.get(key)
                cell = ws.cell(row=rr, column=c0 + j,
                               value=(None if (isinstance(v, float) and pd.isna(v)) else v))
                cell.font = boldf; cell.fill = PatternFill("solid", fgColor=LTBLUE)
                cell.alignment = left if j == 0 else right
                if fmt:
            ...
            for r_ in rows:
                for j, key, fmt in [(0, "name", None), (1, "los", "0.0"),
                    ...
                    cell = ws.cell(row=rr, column=c0 + j,
                                   value=(None if (isinstance(v, float) and pd.isna(v)) else v))
                    cell.font = reg; cell.alignment = left if j == 0 else right
                    cell.border = thin
                    if fmt:
                        cell.number_format = fmt
                rr += 1
            max_len = max(max_len, rr - band_top)
        row = band_top + max_len + 1

    for c, w in [(1, 30), (2, 6), (3, 12), (4, 11), (5, 12),
                 (7, 30), (8, 6), (9, 12), (10, 11), (11, 12),
                 (13, 30), (14, 6), (15, 12), (16, 11), (17, 12)]:
        from openpyxl.utils import get_column_letter
        ws.column_dimensions[get_column_letter(c)].width = w
    ws.sheet_view.showGridLines = False

    # Cancellation review sheet (for PMS fee checks), if provided
    if cancellations is not None and len(cancellations):
        _write_cancellation_sheet(
            wb, cancellations, title="Cancellations",
            header_line=(header_line.replace("Production", "Cancellations")
                         if header_line else "Cancelled Reservations"))

    # Aux sheet with the mapping so it round-trips (re-importable next time)
    ms = wb.create_sheet("Rate Plan Map")
    ms.append(["Rate Code", "Rate Plan Name", "Segment"])
    for _, m in mapping_df.iterrows():
        ms.append([str(m["Rate Code"]), str(m["Rate Plan Name"]), str(m["Segment"])])
    for c, w in [(1, 16), (2, 34), (3, 20)]:
        from openpyxl.utils import get_column_letter
...
def mapping_from_export(sheets):
    """Read a 'Rate Plan Map' sheet back from a prior Rate Plan Production export."""
    name = next((s for s in sheets if s.strip().lower() == "rate plan map"), None)
    if name is None:
        return None
    df = sheets[name]
    df.columns = df.iloc[0]
    df = df.iloc[1:].reset_index(drop=True)
    keep = [c for c in ["Rate Code", "Rate Plan Name", "Segment"] if c in df.columns]
    if len(keep) < 3:
        return None
    out = df[keep].copy()
    out = out[out["Rate Plan Name"].astype(str).str.strip() != ""]
    return out.reset_index(drop=True)



def render_rateplan_page(parts):
    """Streamlit page: Transient Rate Plan Production from a SynXis export."""
    st.title("The Compton — Transient Rate Plan Production")
    st.caption("Rate-plan production by segment (LOS / Room Nights / ADR / Revenue), "
               "built from your SynXis HotelReservations export.")

    resv = parts.get("synxis_resv")
    if resv is None or len(resv) == 0:
        st.info("Upload your **SynXis HotelReservations** export (CSV) in the "
                "sidebar under *Rate Plan Production* to build this report.")
        st.stop()

    # ---- master mapping (seed -> session; re-importable from a prior export) ----
    if "rpp_map" not in st.session_state:
        st.session_state["rpp_map"] = rate_plan_mapping_default()
    mapping_df = st.session_state["rpp_map"]

    months = ["All months"] + available_months(resv)
    with st.sidebar:
        st.markdown("---")
        st.header("Rate Plan settings")
        month_label = st.selectbox("Month (by arrival)", months, index=0)

    prod = compute_rate_plan_production(resv, mapping_df, month_label)

    # ---- flag unmapped rate plans so they can be assigned a segment ----
    if prod["unmapped"]:
        st.warning(f"⚠️ **{len(prod['unmapped'])} rate plan(s) in your export are not "
                   "mapped yet.** Assign a name + segment below and they'll flow into "
                   "the report (and be remembered for this session).")
        u_df = pd.DataFrame(
            [{"Rate Code": c, "SynXis Category": cat,
              "Rate Plan Name": c, "Segment": "Retail",
              "Room Nights": rn, "Revenue": rev}
             for c, cat, rn, rev in prod["unmapped"]])
        edited = st.data_editor(
            u_df, hide_index=True, use_container_width=True, key="rpp_unmapped",
            column_config={
                "Room Nights": st.column_config.NumberColumn(disabled=True),
                "Revenue": st.column_config.NumberColumn(disabled=True, format="$%.2f"),
                "SynXis Category": st.column_config.TextColumn(disabled=True),
                "Segment": st.column_config.SelectboxColumn(options=SEGMENT_ORDER,
                                                            required=True),
            })
        if st.button("➕ Add these to the mapping"):
            add = edited[["Rate Code", "Rate Plan Name", "Segment"]].copy()
            add = add[add["Rate Plan Name"].astype(str).str.strip() != ""]
            st.session_state["rpp_map"] = pd.concat(
                [st.session_state["rpp_map"], add], ignore_index=True
            ).drop_duplicates(subset=["Rate Code"], keep="last")
            st.rerun()
    else:
        st.success("✅ All rate plans in your export are mapped.")

    hdr = f"The Compton — Transient Rate Plan Production — {month_label}"
    st.markdown(render_rateplan_html(prod, hdr), unsafe_allow_html=True)

    st.download_button(
        "⬇️ Rate Plan Production (Excel)",
        data=rateplan_to_excel(prod, st.session_state["rpp_map"], hdr,
                               cancellations=cancellation_review_df(resv, month_label)),
        file_name=f"Compton_RatePlanProduction_{pd.Timestamp.today():%Y%m%d}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )

    n_canc = len(cancellation_review_df(resv, month_label))
    if n_canc:
        st.caption(f"❌ This file has **{n_canc} cancelled reservation(s)** — see the "
                   "**Cancellations** view (top of the sidebar) to review them for "
                   "PMS fees. They're also on a separate tab in the Excel export.")

    with st.expander("✏️ Edit the full rate-plan mapping (Code → Name → Segment)"):
        st.caption("The mapping is seeded from your workbook. Edits apply this "
                   "session; the downloaded Excel embeds a 'Rate Plan Map' sheet you "
                   "can re-upload next time to restore it.")
        new_map = st.data_editor(
            st.session_state["rpp_map"], num_rows="dynamic",
            use_container_width=True, key="rpp_map_editor",
            column_config={"Segment": st.column_config.SelectboxColumn(
                options=SEGMENT_ORDER, required=True)})
        if st.button("Save mapping changes"):
            st.session_state["rpp_map"] = new_map.reset_index(drop=True)
            st.rerun()


def render_cancellations_page(parts):
    """Streamlit page: cancelled reservations, for PMS fee review."""
    st.title("The Compton — Cancelled Reservations")
    st.caption("Cancelled reservations from your SynXis export, for reviewing in "
               "the PMS to see whether cancellation fees are owed.")

    resv = parts.get("synxis_resv")
    if resv is None or len(resv) == 0:
        st.info("Upload your **SynXis HotelReservations** export (CSV) in the "
                "sidebar under *Rate Plan Production*. Export with **Confirmed + "
                "Cancelled** statuses so cancellations appear here (Ignored rows "
    ...
    with st.sidebar:
        st.markdown("---")
        st.header("Cancellation settings")
        month_label = st.selectbox("Month (by arrival)", months, index=0,
                                   key="canc_month")

    canc_df = cancellation_review_df(resv, month_label)
    n_canc = len(canc_df)
    if n_canc == 0:
        st.success("✅ No cancelled reservations in this file"
                   + ("" if month_label == "All months" else f" for {month_label}")
                   + ". If you expected some, re-export from SynXis including the "
                   "**Cancelled** status.")
        st.stop()

    canc_rn = float(canc_df["Room Nights"].sum())
    canc_val = float(canc_df["Value"].sum())
    _dba = pd.to_numeric(canc_df["Days Before Arrival"], errors="coerce")
    _after = int((_dba < 0).sum())

    m1, m2, m3, m4 = st.columns(4)
    m1.metric("Cancellations", f"{n_canc:,}")
    m2.metric("Room nights", f"{canc_rn:,.0f}")
    m3.metric("Value", f"${canc_val:,.2f}")
    m4.metric("Cancelled after arrival", f"{_after:,}",
              help="Days Before Arrival is negative — these were cancelled on/after "
                   "the arrival date and are the most likely to carry a fee.")

    st.caption("**Days Before Arrival** = arrival date − cancelled date (negative "
               "means cancelled on/after arrival). Sorted by arrival date.")
    st.dataframe(
        canc_df, use_container_width=True, hide_index=True,
        column_config={
            "ADR": st.column_config.NumberColumn(format="$%.2f"),
            "Value": st.column_config.NumberColumn(format="$%.2f"),
            "Room Nights": st.column_config.NumberColumn(format="%.0f"),
            "Days Before Arrival": st.column_config.NumberColumn(format="%d"),
        })

    st.download_button(
        "⬇️ Cancellations (Excel)",
        data=cancellations_to_excel(
            canc_df, f"The Compton — Cancelled Reservations — {month_label}"),
        file_name=f"Compton_Cancellations_{pd.Timestamp.today():%Y%m%d}.xlsx",
        mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    )


# =========================================================================== #
#  DELPHI GRC (Group Rooms Control) — tentative group blocks, per-date notes
# =========================================================================== #
_GRC_MONTHS = {m.lower(): i for i, m in enumerate(calendar.month_name) if m}
_GRC_RCT = {"Agr", "Fcs", "Blk", "Pck"}


def _grc_month_year(s):
    import re
    m = re.search(r"([A-Za-z]+)\s*-\s*(\d{4})", str(s or ""))
    if m and m.group(1).lower() in _GRC_MONTHS:
        return _GRC_MONTHS[m.group(1).lower()], int(m.group(2))
    return None, None


def parse_grc_excel(file):
    """Read a Delphi GRC Excel export of TENTATIVE bookings. Returns a list of
    group dicts { name, month, year, rct, days:{date->rooms}, adr, rooms, rev }.
    Reads the day-number header row to map each column to its date, so the
    per-date room-nights land on the correct occupancy date."""
    import datetime as _dt
    from openpyxl import load_workbook
    try:
        wb = load_workbook(file, data_only=True)
    except Exception as e:
        st.sidebar.warning(f"Couldn't read the GRC file ({e}).")
        return []
    groups = []
        ...
        for rr in range(1, maxr + 1):
            ...
            if str(b).strip() == "RCT":
                ...
                if mapping:
                    col2day = mapping
                continue
            if str(b).strip() in _GRC_RCT and col2day and month:
                name = str(a or "").strip()
                if not name or name.lower().endswith("totals"):
                    continue
                days = {}
                for c, d in col2day.items():
                    ...
                    if isinstance(v, (int, float)) and v:
                        try:
                            days[_dt.date(year, month, d)] = float(v)
                        except ValueError:
                            pass
                tail = [ws.cell(rr, c).value for c in range(3, maxc + 1)]
                nums = [t for t in tail if isinstance(t, (int, float))]
                adr = rooms = rev = None
                if len(nums) >= 3:
                    adr, rooms, rev = nums[-3], nums[-2], nums[-1]
                if days:
                    groups.append({"name": name, "month": month, "year": year,
                                   "rct": str(b).strip(), "days": days,
                                   "adr": adr, "rooms": rooms, "rev": rev})
    return groups


def grc_notes_by_date(groups):
    """Collapse parsed GRC groups into { datetime.date : note_text } for the DD.
    Each date's note lists every tentative group with rooms that day + ADR, plus
    a daily total."""
    by_date = {}
    for g in (groups or []):
        adr = g.get("adr")
        for d, rooms in g["days"].items():
            by_date.setdefault(d, []).append((g["name"], rooms, adr))
    notes = {}
    for d, items in by_date.items():
        items.sort(key=lambda x: -x[1])
        total = sum(r for _, r, _ in items)
        lines = [f"TENTATIVE groups — {total:,.0f} rm total"]
        for name, rooms, adr in items:
            adr_s = f" @ ${adr:,.2f}" if isinstance(adr, (int, float)) else ""
            lines.append(f"• {name}: {rooms:,.0f} rm{adr_s}")
        notes[d] = "\n".join(lines)
    return notes
...
with st.sidebar:
    page = st.radio("View", ["Dashboard", "Pick-Up Report (DD)",
                             "Rate Plan Production", "Cancellations"], index=0,
                    help="Switch between the owner dashboard, the daily pick-up "
                         "report, the transient rate-plan production report, "
                         "and the cancelled-reservations review.")
    st.markdown("---")
    ...
    st.markdown("---")
    st.subheader("2 - 7-Day report add-ons")
    st.caption("Optional extras for the 7-day report. The two comparison slots "
               "drive pickup on **both** the Dashboard and the DD (each accepts a "
               "raw IDeaS **Market Segment** export **or** a **previous Compton "
               "export**). The GRC slot flags tentative group demand on each date.")
    ...
    cmp_wk_file = st.file_uploader("② 7 days ago — Market Segment or Compton export",
                                   type=["xlsx"], accept_multiple_files=False,
                                   key="cmp_wk_up")
    grc_file = st.file_uploader("③ Delphi GRC — Tentative groups (Excel)",
                                type=["xlsx"], accept_multiple_files=False,
                                key="grc_up",
                                help="Delphi Group Rooms Control export (Status: "
                                     "Tentative). Adds a pop-out note on each DD "
                                     "date that has tentative group room nights, "
                                     "with group name, day-by-day rooms and ADR.")

    st.markdown("---")
    st.subheader("Rate Plan Production")
    synxis_file = st.file_uploader(
        "SynXis HotelReservations export (CSV)", type=["csv", "xlsx"],
        accept_multiple_files=False, key="synxis_up",
        help="Powers the Transient Rate Plan Production report.")
    rpp_map_file = st.file_uploader(
        "Optional: prior Rate Plan Production export (to restore the mapping)",
        type=["xlsx"], accept_multiple_files=False, key="rpp_map_up")

# The Rate Plan Production / Cancellations pages only need the SynXis file.
if page in ("Rate Plan Production", "Cancellations"):
    if synxis_file is None:
        st.title(f"The Compton — {page}")
        st.info("Upload your **SynXis HotelReservations** export (CSV) in the "
                "sidebar under *Rate Plan Production* to build this report.")
        st.stop()
    _parts = {"synxis_resv": parse_synxis_reservations(synxis_file)}
    # restore a saved mapping from a prior export, if provided
    if rpp_map_file is not None and "rpp_map" not in st.session_state:
        try:
            _m = mapping_from_export(_read_all_sheets(rpp_map_file))
            if _m is not None:
                st.session_state["rpp_map"] = _m
        except Exception as e:
            st.sidebar.warning(f"Couldn't read that mapping file ({e}).")
    if page == "Cancellations":
        render_cancellations_page(_parts)
    else:
        render_rateplan_page(_parts)
    st.stop()
...
# Parse the Delphi GRC tentative-groups export into per-date pop-out notes.
parts["grc_groups"] = parse_grc_excel(grc_file) if grc_file is not None else []
parts["grc_notes"] = grc_notes_by_date(parts["grc_groups"])

# If a SynXis reservations file was provided, parse it so the Rate Plan
# Production sheet can be folded into the combined workbook from any page.
parts["synxis_resv"] = (parse_synxis_reservations(synxis_file)
                        if synxis_file is not None else None)
...
# Only surface a notice if an EXPECTED file is missing (no "OK" clutter).
_missing = [lbl for lbl, key in [("Data Extract", "data"), ("PCDC", "pcdc"),
                                  ("Market Seg", "mseg"), ("Rate Shop", "shop")]
            for lb in [lbl] if parts.get(key) is None]
if "Data Extract" in _missing:
    st.error("The **Data Extract** file wasn't found — it's required to build the report.")
elif _missing:
    st.warning("Not detected (report will still build without them): "
...
# =============================================================== #
#  Scope: default to the PCDC pace window (the dates the PCDC covers)
#  when a PCDC is uploaded, otherwise show every date in the report.
#  No on-screen filters — filter in Excel after downloading.
# =============================================================== #
if _pcdc_min is not None:
    v = report[(report["Date"] >= _pcdc_min) & (report["Date"] <= _pcdc_max)]
else:
    v = report
...
# Highlight the Notes column so entries stand out
def _notes_bg(series):
    return ["background-color:#E7F0FA;color:#1F3A5F;font-weight:600" if str(v).strip()
            and str(v).strip().lower() != "nan" else "" for v in series]

if "📝 Notes" in view.columns:
    styler = styler.apply(_notes_bg, subset=["📝 Notes"])

st.dataframe(styler, use_container_width=True, height=560)

# filename reflects the report's date range
if len(view) > 0:
    _fn_start, _fn_end = view["Date"].min(), view["Date"].max()
else:
    _fn_start, _fn_end = report["Date"].min(), report["Date"].max()

_grc_notes = parts.get("grc_notes") or {}
if _grc_notes:
    _n_grc = sum(1 for d in _grc_notes if _fn_start <= d <= _fn_end)
    st.caption(f"🗓️ **{_n_grc}** date(s) in this range have **tentative group** "
               "demand — hover the flagged Date cells in the Excel export for the "
               "group name, day-by-day rooms and ADR.")

_ddl1, _ddl2 = st.columns(2)
_ddl1.download_button(
    "⬇️ Pick-Up Report only (Excel)",
    data=to_excel(view, grc_notes=_grc_notes),
    file_name=f"Compton_PickUp_{_fn_start:%Y%m%d}_{_fn_end:%Y%m%d}.xlsx",
    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
    use_container_width=True,
)

# Combined workbook — Dashboard + DD Pick-Up Report (+ Rate Plan Production if a
# SynXis file was uploaded). The Dashboard is built from the uploaded Market
# Segment + comparison files (no extra uploads).
...
if _dash_export is not None:
    ...
    if _resv is not None and len(_resv):
        ...
        _rp_prod = compute_rate_plan_production(_resv, _rp_map, "All months")
        _rp_canc = cancellation_review_df(_resv, "All months")
        _rp_hdr = "The Compton — Transient Rate Plan Production — All months"
    _combined = build_combined_excel(
        _dtables, view, header_line=_dhdr,
        show_lr=_dlr, show_week=_dweek,
        rp_prod=_rp_prod, rp_mapping=_rp_map, rp_header=_rp_hdr,
        rp_cancellations=_rp_canc, grc_notes=_grc_notes)
    _lbl = ("⬇️ DD + Dashboard + Rate Plan (one Excel)" if _rp_prod is not None
            else "⬇️ DD + Dashboard (one Excel)")

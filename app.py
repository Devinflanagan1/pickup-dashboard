...
# Seed the as-of value ONLY the very first time this widget is ever created in
# this browser session. After that, st.date_input owns the value and a manual
# "Reset to today" button (below) is the only other thing allowed to change it —
# this avoids any ambiguity about what set the date.
...
with st.sidebar:
    ...
    as_of = st.date_input(
        ...
        key="as_of_input",
        help="Defaults to today's real date. Change it here only if you want to "
             "look at pace as of a different reference date.",
    )
    if st.button("🔄 Reset as-of date to today", use_container_width=True):
        st.session_state["as_of_input"] = real_today
        st.rerun()

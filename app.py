...
def build_report(parts, as_of, comp_method="Average", show_change="None", thr=None,
    ...
    # --- 🔥 Pickup Alert: flags an unusually large day's pickup ------------- #
    ...
    def _pickup_label(i):
        if bool(is_big_pickup.iloc[i]):
            v = pickup.iloc[i]
            return f"🔥 Big Pickup (+{v:,.0f})"
        return ""

    r["🔥 Pickup Alert"] = [_pickup_label(i) for i in range(len(r))]

    # ---- Enforce the exact original report column order ---- #
    lead = ["⚡ Action", "🔥 Pickup Alert", "DOW", "Date", "Days Left", "Events",
...
def to_excel(r, kpi_ctx=None):
    ...
    def _pickup_fill(txt):
        return PatternFill("solid", fgColor="FDE7CE") if "Big Pickup" in str(txt) else None
    ...
    with st.expander("🔴 Rate leadership (#1 vs #2)", expanded=False):
        ...
        lead_buffer = st.slider("Minimum $ cushion we want over the #2 (top competitor)",
                                ...
                                help="If we're #1 but ahead by less than this, flag "
                                     ...
                                     "of this slider.")
    with st.expander("🔥 Big pickup alert", expanded=False):
...
pickup_only = fg.checkbox("🔥 Big pickup days only", value=False)
...
if pickup_only and "🔥 Pickup Alert" in v:
    v = v[v["🔥 Pickup Alert"].astype(str).str.strip() != ""]
...
# Colour the 🔥 Pickup Alert column so big pickup days pop
def _pickup_bg(series):
    return ["background-color:#FDE7CE;color:#B45309;font-weight:600" if "Big Pickup" in str(v)
            else "" for v in series]

if "🔥 Pickup Alert" in view.columns:
    styler = styler.apply(_pickup_bg, subset=["🔥 Pickup Alert"])

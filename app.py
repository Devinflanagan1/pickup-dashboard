...
def to_excel(r):
    ...
    while j < n:
        l1 = lv[j][0]
        k = j
        while k + 1 < n and lv[k + 1][0] == l1 and lv[j][1]:  # only span multi-col groups
            k += 1
...
otb_sold   = "Rooms Sold | Total Hotel | Current"    # the OTB heat-map column
...
# Heat map (background gradient) on the OTB rooms sold column
if otb_sold in view.columns and view[otb_sold].notna().any():
    try:
        styler = styler.background_gradient(cmap="RdYlGn", subset=[otb_sold])
    except Exception:
        pass

st.dataframe(styler, use_container_width=True, height=560)

# ---- Calendar heat chart for OTB rooms sold ---- #
if otb_sold in view.columns and view[otb_sold].notna().any():
...
| Rooms OTB STLY (Hotel/Transient/Group, Current + Change) | Data Extract | On-Books STLY; Change = TY − STLY |

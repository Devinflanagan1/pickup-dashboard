...
def build_report(parts, as_of, comp_method="Average", show_change="None"):
    ...
    lead = ["DOW", "Date", "Days Left", "Events", "Rooms Left to Sell",
            "BAR", "Comp Set Avg", "Last Room Value"]
...
def to_excel(r):
    ...
    if otb_sold_col is not None and len(r) > 0:
        ...
        # GREEN = low occupancy, RED = high / near sell-out
        ws.conditional_formatting.add(
            rng,
            ColorScaleRule(start_type="min", start_color="63BE7B",         # green (low)
...
def _heat_bg(series):
    # GREEN = low occupancy (lots of rooms open), RED = high / near sell-out.
    ...
    for v in vals:
        if pd.isna(v):
            out.append("")
            continue
        t = 1 - (v - lo) / span                # invert: high value -> red end
        if t < 0.5:
            f = t / 0.5

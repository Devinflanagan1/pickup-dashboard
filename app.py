...
if df_pcdc_biz is None or df_extract is None:
    ...
    # Identify date columns and parse with errors='coerce' to skip invalid text rows safely
    date_col_extract = next(
        (c for c in df_extract.columns if "Date" in str(c)), df_extract.columns[0]
    )
    df_extract["Date_Clean"] = pd.to_datetime(
        df_extract[date_col_extract], errors="coerce"
    )
    df_extract = df_extract.dropna(subset=["Date_Clean"])

    date_col_pcdc = next(
        (c for c in df_pcdc_biz.columns if "Date" in str(c)),
        df_pcdc_biz.columns[0],
    )
    df_pcdc_biz["Date_Clean"] = pd.to_datetime(
        df_pcdc_biz[date_col_pcdc], errors="coerce"
    )
    df_pcdc_biz = df_pcdc_biz.dropna(subset=["Date_Clean"])

...
def build_combined_excel(tables, report, header_line=None,
    ...
    Report, and (optionally) the Transient Rate Plan Production. Any section is
    optional — pass None to skip it.
    Sheet order: Dashboard, Pick-Up Report, Rate Plan Production (+ its map)."""
    from openpyxl import Workbook
    wb = Workbook()
...
def render_dashboard_page(parts, as_of):
    ...
    if parts.get("data") is not None:
        try:
            ...
            _notes_cur = st.session_state.get("dd_notes", {})
...
def parse_synxis_reservations(file):
    ...
    Returns a DataFrame with: code, category, arrival, nights, rooms, rate, RN, Rev, ym.
    ...
    if c_code is None or c_nights is None or c_rate is None:
        return pd.DataFrame(columns=["code", "category", "arrival", "nights",
                                     "rooms", "rate", "RN", "Rev", "ym"])

    if c_status is not None:
        df = df[df[c_status].astype(str).str.strip().str.lower() == "confirmed"]
...
def compute_rate_plan_production(resv, mapping_df, month_label="All months"):
    """Group reservations into the rate-plan/segment layout.
...
def rateplan_to_excel(prod, mapping_df, header_line=None, wb=None):
    from openpyxl import Workbook
    from openpyxl.styles import Font, Alignment, PatternFill, Border, Side
...
with st.sidebar:
    page = st.radio("View", ["Pick-Up Report (DD)", "Dashboard",
                             "Rate Plan Production"], index=0,
                    help="Switch between the daily pick-up report, the owner "
                         "dashboard, and the transient rate-plan production report.")
    st.markdown("---")
...
# The Rate Plan Production page only needs the SynXis file, not the four xlsx.
if page == "Rate Plan Production":
...
# =============================================================== #
#  Notes (manual, session-only — do NOT carry forward)
# =============================================================== #
if "dd_notes" not in st.session_state:
    st.session_state["dd_notes"] = {}

with st.expander("📝 Notes (manual, per date)", expanded=False):
    st.caption("Free-text notes that appear in the **📝 Notes** column next to "
               "🔥 Spike. These are session-only and are **not** carried forward "
               "from a prior report.")
    nc1, nc2, nc3 = st.columns([1, 2.4, 0.7])
    _note_date = nc1.date_input("Date", value=as_of, key="note_date")
    _note_text = nc2.text_input("Note", key="note_text",
                                placeholder="e.g. Group block cutoff, event, hold…")
    nc3.markdown("<div style='height:1.7em'></div>", unsafe_allow_html=True)
    if nc3.button("Add", use_container_width=True, key="note_add"):
        nn = dict(st.session_state["dd_notes"])
        ds = pd.Timestamp(_note_date).strftime("%Y-%m-%d")
        if str(_note_text).strip():
            nn[ds] = str(_note_text).strip()
        else:
            nn.pop(ds, None)
        st.session_state["dd_notes"] = nn

    _notes_df = pd.DataFrame(
        [{"Date": pd.Timestamp(d).date(), "Note": v}
         for d, v in sorted(st.session_state["dd_notes"].items())]
    )
    if _notes_df.empty:
        _notes_df = pd.DataFrame({"Date": pd.Series([], dtype="object"),
                                  "Note": pd.Series([], dtype="object")})
    _edited_notes = st.data_editor(
        _notes_df, num_rows="dynamic", use_container_width=True, key="notes_editor",
        column_config={"Date": st.column_config.DateColumn("Date", format="YYYY-MM-DD"),
                       "Note": st.column_config.TextColumn("Note")})
    _nn = {}
    for _, _row in _edited_notes.iterrows():
        _d, _t = _row.get("Date"), _row.get("Note")
        if pd.isna(_d) or not str(_t or "").strip():
            continue
        _nn[pd.Timestamp(_d).strftime("%Y-%m-%d")] = str(_t).strip()
    st.session_state["dd_notes"] = _nn
    st.caption(f"**{len(_nn)}** date(s) with a note.")

_dd_notes = st.session_state.get("dd_notes", {})
...
report = build_report(parts, as_of, comp_method, show_change, thr, show_own_change,
                      ...
                      restr_annotate=_restr_has_prev, notes=_dd_notes)
...
if _dash_export is not None:
    ...
    _resv = parts.get("synxis_resv")
    if _resv is not None and len(_resv):
        _rp_map = st.session_state.get("rpp_map") or rate_plan_mapping_default()
        _rp_prod = compute_rate_plan_production(_resv, _rp_map, "All months")
